...
public class DemoDBManager {
     ...
     * @return return cursor of the message
     */
    public synchronized Integer saveQiusaiMessage(ActiveDetail message) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        int id = -1;
        if (db.isOpen()) {
            ContentValues values = new ContentValues();
            ...
            values.put(QiusaixinxiDao.DELETEFLAG, message.getDeleteFlag());
     ...
     * @param values
     */
    synchronized public void updateQiusaiMessage(int msgId, ContentValues values) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
            db.update(QiusaixinxiDao.TABLE_NAME, values, QiusaixinxiDao.ID + " = ?", new String[]{String.valueOf(msgId)});
        }
    }
     ...
     * @return
     */
    synchronized public List<ActiveDetail> getQiusaiMessagesList(boolean name) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        List<ActiveDetail> msgs = new ArrayList<ActiveDetail>();
        Cursor cursor;
        if (db.isOpen()) {
            if (name) {
                cursor = db.rawQuery("select * from " + QiusaixinxiDao.TABLE_NAME + " where " + QiusaixinxiDao.DELETEFLAG + " ='" + "0" + "'", null);
            } else {
                cursor = db.rawQuery("select * from " + QiusaixinxiDao.TABLE_NAME, null);
            }
            ...
            while (cursor.moveToNext()) {
                ...
                String status = cursor.getString(cursor.getColumnIndex(QiusaixinxiDao.FLAG));
     ...
     * @param from
     */
    synchronized public void deleteQiusaiMessage(String from) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
            db.delete(QiusaixinxiDao.TABLE_NAME, QiusaixinxiDao.ID + " = ?", new String[]{from});
        }
    }
